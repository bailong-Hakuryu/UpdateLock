...
if defined VS_PATH (
    ...
    cl /std:c++17 /EHsc /utf-8 /Fe:build\UpdateLock.exe src\main.cpp src\SecurityManager.cpp src\ProcessManager.cpp src\Immunizer.cpp advapi32.lib shell32.lib user32.lib
    if %errorlevel% equ 0 (
        echo.
...
if %errorlevel% equ 0 (
    echo [✓] 找到 g++ 编译器
    if not exist build mkdir build
    g++ -std=c++17 -O2 src/main.cpp src/SecurityManager.cpp src/ProcessManager.cpp src/Immunizer.cpp -ladvapi32 -lshell32 -luser32 -o build/UpdateLock.exe
    if %errorlevel% equ 0 (
        echo.
        echo [✓] 编译成功！可执行文件路径: build/UpdateLock.exe
    ) else (
